...
export default function HomeBanner() {
  ...
  return (
    <div className="relative home-banner   w-full">
      {data.map((item) => (
        <div key={item.id} className={`${selectedTab.id === item.id ? "block" : "hidden"}`}>
          <Swiper
                ...
                <div className="relative home-banner-swiper sm:!min-h-[700px] flex flex-col justify-end">
                  ...
                  <div className="py-4 !z-[9999] container absolute bottom-0  flex flex-col justify-center items-center   sm:bottom-10 left-1/2 -translate-x-1/2 w-full px-4 sm:px-6 text-white">
                    <div className="w-full">
                      <motion.h2
                      style={{ textShadow: "0px 0px 10px rgba(255,255,255,0.64)" }}
                        initial={{ opacity: 0, y: 20 }}
                        animate={{ opacity: 1, y: 0 }}
                        transition={{ duration: 0.6 }}
                        className="text-[32px] text sm:text-[40px]  md:text-[56px] !font-bold bg-[#16294F] px-4 flex justify-center items-center !shadow-lg h-[50px] sm:h-[60px] xl:h-[70px] rounded-md w-fit mb-2"
                      >
                        {item.main_title}
                      </motion.h2>
                      <motion.p
                      style={{ textShadow: "0px 0px 10px rgba(255,255,255,0.64)" }}
                        ...
                        className="text-sm sm:text-base md:text-lg max-w-full md:max-w-[50%] text-white"
                      >
                        {item.desc}
                      </motion.p>
                    </div>
